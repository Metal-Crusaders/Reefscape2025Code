...
public class L2AndTwoL3AutoOpenSide extends SequentialCommandGroup {

    public L2AndTwoL3AutoOpenSide(CommandSwerveDrivetrain swerve, Elevator elevator, AlgaeClaw algaeClaw, AlgaePivot algaePivot, CoralShooter coralShooter) {
        ...
        Pose2d startingPose;
        try {
            startToL2 = swerve.driveAlongPath(PathPlannerPath.fromPathFile("StartingToL2OpenSide"));
            if (DriverStation.getAlliance().get().equals(DriverStation.Alliance.Blue)) {
                startingPose = PathPlannerPath.fromPathFile("StartingToL2OpenSide").getStartingHolonomicPose().get();
                resetPose = new InstantCommand(() -> swerve.resetPose(startingPose), swerve);
            } else {
                startingPose = PathPlannerPath.fromPathFile("StartingToL2OpenSide").flipPath().getStartingHolonomicPose().get();
                resetPose = new InstantCommand(() -> swerve.resetPose(startingPose), swerve);
            }
        ...
        try {
            l2ToCoralStation = swerve.driveAlongPath(PathPlannerPath.fromPathFile("L2ToCoralStationOpenSide"));
        } catch (IOException | ParseException e) {
            e.printStackTrace();
            l2ToCoralStation = null; // or handle the error appropriately
        }
        try {
            l3ToCoralStation = swerve.driveAlongPath(PathPlannerPath.fromPathFile("L3ToCoralStationOpenSideWithAlgae"));
        } catch (IOException | ParseException e) {
            e.printStackTrace();
        ...
        try {
            coralStationToL32 = swerve.driveAlongPath(PathPlannerPath.fromPathFile("CoralStationToL3-2OpenSide"));
        } catch (IOException | ParseException e) {
            e.printStackTrace();
